	...
	while (true) {
		for (Entity& entity1 : entities) {
			for (auto i = entities.begin(); i != entities.end(); ++i) {
				...
				entity1.velocity.x += entity1.mass * entity2.mass / distanceX /* * std::abs(distanceX) */ / entities.size();
				entity1.velocity.y += entity1.mass * entity2.mass / distanceY /* * std::abs(distanceY) */ / entities.size();
				entity1.velocity.x *= 0.99;
				entity1.velocity.y *= 0.99;
			}
		}
		...
		for (Entity& entity : entities) {
			...
			entity.position.x = std::fmod(entity.position.x + entity.velocity.x + screenSize.column, screenSize.column);
			entity.position.y = std::fmod(entity.position.y + entity.velocity.y + screenSize.row, screenSize.row);
		}
